export default function(ngapp) {
    ...
    var Clusterize = function(data) {
        ...
        // public parameters
        self.options = {};
        var options = ['rows_in_block', 'blocks_in_cluster', 'show_no_data_row', 'no_data_class', 'no_data_text', 'keep_parity', 'tag', 'callbacks', 'item_height'];
    ...
    ngapp.directive('mainTreeNodes', function($compile, $timeout, virtualNodeService) {
        function _buildNodes(nodes) {
            return nodes.map(function(node, index) {
                return `<div ng-if="1" ng-init="node = node.children[${index}]"><main-tree-node></main-tree-node></div>`;
            });
        }

        function _linkFn($scope, $elem) {
            // Clusterize elements
            var scrollEl =  virtualNodeService.findRoot($elem[0]);
            var contentEl = $elem[0].firstElementChild;

            // init clusterize
            var clusterize = new Clusterize({
                rows: _buildNodes($scope.node.children),
                scrollElem: scrollEl,
